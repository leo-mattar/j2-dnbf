...
function fundingCards() {
  $(".c-funding-card").each(function (index) {
    if ($(this).data("flip")) { // Check if the card has the data-flip attribute set to true
      let tl = gsap.timeline({ paused: true });

      tl.to($(this).find(".c-funding-card-item"), {
        rotationY: 180,
        duration: 1,
        ease: "power2.inOut"
      });
...
function menuMobile() {
  ...
  tl.fromTo(
    menuEl, { clipPath: "inset(0% 0% 0% 100%)" }, { clipPath: "inset(0% 0% 0% 0%)" },
    0
  );
